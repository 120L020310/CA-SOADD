...
from models.OneClass.model_cross_domain_wo_binv import CA_SOADD_cross_domain
...
if __name__ == "__main__":
    ...
    parser.add_argument("--root_dir",type=str,default="/home/user/data/Result_cross_domain")
    ...
    # train_ds, train_dl = get_true(["ASV5"],"val",bs = 32)     
    train_ds, train_dl = get_true(["MLAAD"],bs=16,label="train",language_list=["en", "de", "es","fr","it","pl","ru","uk"]) 
    val_ds, val_dl_real = get_true(["MLAAD"],bs=16,label="val",language_list=["en", "de", "es","fr","it","pl","ru","uk"]) 
    args.profiler = (
    ...
    # print(str(dict(cfg)))
    model = CA_SOADD_cross_domain(post=True,language_list=["en", "de", "es","fr","it","pl","ru","uk"])
    ...
    if not args.test:
        ...
        trainer.fit(model,train_dl, val_dataloaders=val_dl_real, ckpt_path=ckpt_path)
